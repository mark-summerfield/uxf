...
fn t_tclass_new() {
    ...
    let bsize = 5;
    let b =
        TClass::new("BType", valid_fields()[..bsize].to_vec(), "").unwrap();
...
fn t_tclass_new_fieldless() {
    let ready = TClass::new_fieldless("ReadyState", "Ready enum").unwrap();
...
fn t_tclass_eq_lt() {
    ...
    let b2 = b.clone();
    let c2 =
        TClass::new("Charlie", valid_fields()[..3].to_vec(), "").unwrap();
...
fn t_tclass_invalid_ttype() {
    for (code, name) in [
        ...
        assert!(tclass.is_err(), "expected err of #{} on {}", code, name);
